...
function draw() {
  ...
  // Initialize with default values if sliders not available
  if (!angleSlider) angleAttack = radians(5);
  if (!windSlider) windSpeed = 70;
  if (!altitudeSlider) altitude = 0;
  if (!massSlider) mass = 80;
...
function updateFlowParticles() {
  for (let i = flowParticles.length - 1; i >= 0; i--) {
    ...
    // Reset particle if it goes too far or gets too old
    if (p.x > 200 || p.x < -200 || p.age > p.maxAge) {
      // Reset to start of wing
      p.x = -180 + random(-20, 20);
      if (p.surface === 'upper') {
        p.y = random(-60, -20);
      } else {
        p.y = random(20, 60);
      ...
      p.age = 0;
      p.vx = p.surface === 'upper' ? 2 + random(1) : 1.5 + random(0.5);
      p.vy = 0;
    }
  }
}
